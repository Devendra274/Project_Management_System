...
public interface StudentRepository extends JpaRepository<Student, Long> {
	...
	@Query("select s from Student s join s.userAccount u join s.project p where u.courseName =:coursename order by s.prn")
	List<Student> findAllByOrderByPrn(@Param("coursename") Course coursename);
	
	//List<Student> findAllByCourse(Course coursename);
